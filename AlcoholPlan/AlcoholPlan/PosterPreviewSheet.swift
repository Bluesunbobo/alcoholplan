...
struct PosterPreviewSheet: View {
    @Binding var images: [UIImage?] // Changed to Binding for dynamic updates
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 24) {
                    ...
                    // The Image Layer with Lazy Loading Support
                    ...
                    VStack(spacing: 12) {
                        ...
                        Button(action: saveToPhotos) {
                            ...
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 16)
                            .background(images[selectedIndex] == nil ? Color.primary.opacity(0.3) : Color.primary)
                            .clipShape(Capsule())
                        }
                        .disabled(isSaving || images[selectedIndex] == nil)
                        ...
                        Button(action: {
                            ...
                            .foregroundColor(.primary.opacity(images[selectedIndex] == nil ? 0.3 : 0.8))
                            .frame(maxWidth: .infinity)
                            .padding(.vertical, 12)
                            .background(
                                Capsule()
                                    .stroke(Color.primary.opacity(images[selectedIndex] == nil ? 0.1 : 0.3), lineWidth: 1)
                            )
                        }
                        .disabled(images[selectedIndex] == nil)
                    }
                    .padding(.horizontal, 24)
                    .padding(.bottom, 32)
                }
    ...
    private func saveToPhotos() {
        guard selectedIndex < images.count, let img = images[selectedIndex] else { return }
        isSaving = true
        PosterRenderer.shared.saveImageToPhotos(img) { success, error in
    ...
    private func presentSystemShare() {
        ...
        let topVC = PosterRenderer.shared.getTopViewController()
