...
struct ContentView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Check for photo library permission at startup
            PosterRenderer.shared.checkAndRequestStartupPermission()
        }
    }
...
struct DashboardView: View {
    ...
    @State private var previewImages: [UIImage?] = [] // Updated to optional for lazy loading
    @State private var showingShareOptions = false
    @State private var showingHistorySelection = false
    @State private var isGeneratingPoster = false  // NEW: Loading state
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingPreview) {
            if !previewImages.isEmpty {
                PosterPreviewSheet(images: $previewImages) // Passed as Binding
    ...
    private func generateLivePoster() {
        // 1. Immediately prepare slots and show preview to ensure 'Zero-Latency' pop
        let hasDrinks = !brain.drinks.isEmpty
        self.previewImages = hasDrinks ? [nil, nil, nil] : [nil, nil]
        
        withAnimation {
            self.showingPreview = true
        }
        
        // 2. Perform all rendering in background tasks
        Task {
            // A. FASTEST: Snapshot
            let momentImg = await PosterRenderer.shared.renderMomentPoster(brain: brain, userSettings: userSettings)
            await MainActor.run { 
                withAnimation { self.previewImages[0] = momentImg }
            }
            
            // B. MEDIUM: Session Summary
            if hasDrinks {
                let sessionImg = await PosterRenderer.shared.renderSessionSummaryPoster(brain: brain, userSettings: userSettings)
                await MainActor.run {
                    withAnimation { self.previewImages[1] = sessionImg }
                }
            }
            
            // C. SLOWEST: Heatmap
            let heatmapIndex = hasDrinks ? 2 : 1
            let heatmapImg = await PosterRenderer.shared.renderAnnualHeatmapPoster(targetYear: nil)
            await MainActor.run {
                withAnimation { self.previewImages[heatmapIndex] = heatmapImg }
            }
        }
    }
                    ...
                    VStack(spacing: 32) {
                        ...
                        if sessions.isEmpty {
                            ...
                            HistoryTimelineView(sessions: sessions, brain: brain, userSettings: userSettings, onDelete: { session in
                                ...
                                Task {
                                    // A. Session Poster
                                    let sessionImg = await PosterRenderer.shared.renderHistoricalSessionPoster(session: session, brain: brain, userSettings: userSettings)
                                    await MainActor.run {
                                        withAnimation { self.previewImages[0] = sessionImg }
                                    }
                                    
                                    // B. Heatmap Poster (Heavy)
                                    let heatmapImg = await PosterRenderer.shared.renderAnnualHeatmapPoster(targetYear: sessionYear)
                                    await MainActor.run {
                                        withAnimation { self.previewImages[1] = heatmapImg }
                                    }
                                }
                            })
                        }
